...
class MY_Model extends CI_Modal{
	/* --------------------------------------------------------------
     * VARIABLES
     * ------------------------------------------------------------ */
	...
	/**
	 * Get result search from your keyword.
	 */
	 	
	 public function search($key,$field,$order='DESC'){
		$this->_database->like($field,$key);
		$this->_database->order_by($field,$order);
		$row = $this->_database->get($this->_table)->{$this->_return_type('array')}();
        $this->_temporary_return_type = $this->return_type;
        $this->_with = array();
        return $row;
	}
	...
	/**
	 * Get result search from your keyword with index and limit data.
	 * Order can be custome and limit data to show can custom too..
	 */
	 	
	 public function get_search($key,$field,$start='0',$limit='10',$order='DESC'){
		$this->_database->like($field,$key);
		$this->_database->order_by($field,$order);
